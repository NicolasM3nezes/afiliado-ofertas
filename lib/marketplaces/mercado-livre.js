...
function sellerReputationScore(value) {
  ...
  if (reputation.includes("green")) return 18;
  if (reputation.includes("yellow")) return 11;
  if (reputation.includes("orange")) return 6;
...
function calculateCatalogScore({
  ...
  const discountScore = clamp(Number(discountPercent || 0) / 50, 0, 1) * 30;
  const shippingScore = freeShipping ? 12 : 0;
  const officialScore = officialStore ? 14 : 4;
  const conditionScore = condition === "new" ? 7 : 0;
  const relevanceScore = clamp(Number(relevance || 0), 0, 1) * 14;
...
function normalizeCatalogOffer(product, listing, query) {
  if (!product || !listing) return null;

  const price = Number(listing.price || 0);
  const originalPrice = Number(listing.original_price || 0) || null;
  ...
  const hasDeal = Array.isArray(listing.deal_ids) && listing.deal_ids.length > 0;
  ...
  return {
    ...
    couponText: hasDeal ? "Oferta ativa no Mercado Livre" : null,
    raw: {
...
async function resolveCatalogOffer(candidate, query, accessToken) {
  ...
  if (detail?.buy_box_winner) {
    const winnerOffer = normalizeCatalogOffer(product, detail.buy_box_winner, query);
    if (
      winnerOffer
      && winnerOffer.discountPercent > 0
  ...
  return chooseBestListing(product, discountedListings, query);
}
...
export async function searchMercadoLivreBest({
  ...
  return {
    ...
    warning: ranked.length
      ? null
      : "O Mercado Livre respondeu pelo catálogo, mas não encontrei ofertas com desconto suficiente para esta palavra-chave.",
    pagesFetched: 1,
    status: 200,
    source: "mercado_livre_catalog",
  };
