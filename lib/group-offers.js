...
function publicationLink(offer) {
  return offer?.shortUrl
    || offer?.short_url
    || offer?.metadata?.short_url
    || offer?.metadata?.meli_short_url
    || offer?.affiliateUrl
    || offer?.affiliate_url
    || offer?.permalink
    || "";
...
function couponLine(value) {
  const text = String(value || "").trim();
  if (!text) return "";
  const genericPromotion = /preço promocional|preco promocional|promoção no|promocao no|oferta no mercado/i.test(text);
  return genericPromotion ? `🏷️ ${text}` : `🎟️ Cupom: *${text}*`;
}

export function renderGroupMessage(template, offer) {
  const link = publicationLink(offer);
  ...
  const couponText = offer?.couponText || offer?.coupon_text || offer?.metadata?.coupon_text || "";
  const seller = String(offer?.sellerName || offer?.seller_name || "").trim();
  ...
  const priceLine = originalPrice > price
    ? `💰 De ~${formatMoney(originalPrice)}~ por *${formatMoney(price)}* 👊`
  ...
  const replacements = {
    ...
    "{coupon}": couponLine(couponText),
    "{seller}": seller ? `🏪 ${seller}` : "",
    "{sold}": sold > 0 ? `🔥 +${sold.toLocaleString("pt-BR")} vendidos` : "",
...
export function offerSnapshot(offer, groupTemplate = "") {
  return {
    ...
    metadata: {
      ...
      coupon_text: offer.couponText || offer.coupon_text || offer.metadata?.coupon_text || null,
      short_url: offer.shortUrl || offer.short_url || offer.metadata?.short_url || offer.metadata?.meli_short_url || null,
    },
  };
}
